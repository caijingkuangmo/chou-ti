...
class CouponRecord(models.Model):
    """优惠券发放、消费纪录"""
    coupon = models.ForeignKey("Coupon", on_delete=models.CASCADE)  #绑定到某一类优惠券，而不是具体某张
    number = models.CharField(max_length=64, unique=True, verbose_name="优惠券编号")
...
class CourseSchedule(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = "课程模块计划表（学位课）"
...
class StudyRecord(models.Model):
    ...
    class Meta:
        verbose_name_plural = "学习记录表（报名学位课程后，每个模块会立刻生成一条学习纪录）"
...
class StuFollowUpRecord(models.Model):
    """学员跟进记录"""
    enrolled_degree_course = models.ForeignKey("EnrolledDegreeCourse", verbose_name="报名课程记录", on_delete=models.CASCADE)
